...
import CodeRainEffect from '../effects/CodeRainEffect';
import MathRainEffect from '../effects/MathRainEffect';
import ThermodynamicVisualizer from '../effects/ThermodynamicVisualizer';
...
export default class SceneManager {
    ...
    constructor() {
        ...
        /** @type {CodeRainEffect | null} The code rain effect instance */
        this.codeRainEffect = null;
        /** @type {MathRainEffect | null} The math rain effect instance */
        this.mathRainEffect = null;
        /** @type {ThermodynamicVisualizer | null} The thermodynamic visualizer instance */
        this.thermodynamicVisualizer = null;
    ...
    _createParticleEffect() {
        ...
        // Create thermodynamic visualizer after mesh is ready
        this._createThermodynamicVisualizer();
    }
    
    /**
     * Creates the code rain effect.
     * @private
     */
    _createCodeRainEffect() {
        const initialColor = {
            red: this.uniforms.u_red.value,
            green: this.uniforms.u_green.value,
            blue: this.uniforms.u_blue.value
        };
        this.codeRainEffect = new CodeRainEffect(this.scene, {
            columnCount: 50,
            speed: 0.5
        });
        this.codeRainEffect.setColor(initialColor);
    }
    
    /**
     * Creates the math rain effect (TSU equations).
     * @private
     */
    _createMathRainEffect() {
        const initialColor = {
            red: this.uniforms.u_red.value,
            green: this.uniforms.u_green.value,
            blue: this.uniforms.u_blue.value
        };
        this.mathRainEffect = new MathRainEffect(this.scene, {
            columnCount: 30,
            speed: 0.4
        });
        this.mathRainEffect.setColor(initialColor);
    }
    
    /**
     * Creates the thermodynamic visualizer (pbits + energy-based models).
     * @private
     */
    _createThermodynamicVisualizer() {
        // Create after mesh is created
        if (this.mesh) {
    ...
    setActiveEffect(effectName) {
        ...
        if (this.codeRainEffect && this.codeRainEffect.particles) {
            this.codeRainEffect.particles.visible = (effectName === 'codeRain');
        }
        if (this.mathRainEffect && this.mathRainEffect.spriteObjects) {
            // Show/hide all sprites
            this.mathRainEffect.spriteObjects.forEach(sprite => {
                sprite.visible = (effectName === 'codeRain');
            });
        }
        
        // Thermodynamic visualizer is always active when icosahedron is visible
        if (this.thermodynamicVisualizer) {
            // Show pbits and energy field when icosahedron is active
            const isActive = (effectName === 'icosahedron');
            if (this.thermodynamicVisualizer.pbitSpheres) {
                this.thermodynamicVisualizer.pbitSpheres.forEach(sphere => {
                    sphere.visible = isActive;
                });
            }
            if (this.thermodynamicVisualizer.energyField) {
                this.thermodynamicVisualizer.energyField.visible = isActive;
            }
        }
    }
    ...
    update(deltaTime, elapsedTime, interactionData) {
        ...
        if (this.activeEffectName === 'icosahedron' && this.mesh) {
            ...
            // Update thermodynamic visualizer when icosahedron is active
            if (this.thermodynamicVisualizer) {
                this.thermodynamicVisualizer.update(deltaTime, elapsedTime, { frequency: audioFrequency });
            }
        ...
        } else if (this.activeEffectName === 'codeRain') {
            // Update both code rain (particles) and math rain (sprites with equations)
            if (this.codeRainEffect) {
                this.codeRainEffect.update(deltaTime, elapsedTime, { frequency: audioFrequency });
            }
            if (this.mathRainEffect) {
                this.mathRainEffect.update(deltaTime, elapsedTime, { frequency: audioFrequency });
            }
        }
    }
    ...
    updateShaderUniforms(newUniforms) {
        ...
        // Update code rain color if the effect exists and colors changed
        if (this.codeRainEffect && (newUniforms.u_red !== undefined || newUniforms.u_green !== undefined || newUniforms.u_blue !== undefined)) {
            const newColor = {
                red: this.uniforms.u_red.value,
                green: this.uniforms.u_green.value,
                blue: this.uniforms.u_blue.value
            };
            this.codeRainEffect.setColor(newColor);
        }
        
        // Update math rain color if the effect exists and colors changed
        if (this.mathRainEffect && (newUniforms.u_red !== undefined || newUniforms.u_green !== undefined || newUniforms.u_blue !== undefined)) {
            const newColor = {
                red: this.uniforms.u_red.value,
                green: this.uniforms.u_green.value,
                blue: this.uniforms.u_blue.value
            };
            this.mathRainEffect.setColor(newColor);
        }
    }
